...
if(!function_exists('nfpndx_plugin_deactivate')){

	function nfpndx_plugin_deactivate() {
		...
		if(!empty($nfproot_plugins_settings['nfpndx'])) {		
					
			//unset plugin root settings
			unset($nfproot_plugins_settings['nfpndx']);
						
		}

		//if, after cleaning nfpndx settings, root settings is empty, delete it (no more NutsForPress plugin is activated)
	...
	error_log('NUTSFORPRESS ERROR: function "nfpndx_plugin_deactivate" already exists');
